...
class OrderManager:
    ...
    def create_and_post_order(
        ...
        try:
            ...
            # 2. Create and Sign the order (using the SDK inside PolyClient)
            signed_order = self.client.sdk.create_order(order_args)
            
            # 3. Post the order
            response = self.client.sdk.post_order(signed_order, orderType=order_type)
    ...
    def cancel_order(self, order_id: str) -> dict:
        ...
        try:
            ...
            response = self.client.sdk.cancel_order(order_id)
    ...
    def cancel_all_orders(self) -> dict:
        ...
        try:
            ...
            response = self.client.sdk.cancel_all()
            return {"status": "success", "response": response}
        except Exception as e:
    ...
    def is_order_scoring(self, order_id: str) -> bool:
        """
        Checks if an order is currently within the scoring range for rewards.
        """
        try:
            # Note: order_id param in SDK is 'order_id' (with underscore)
            res = self.client.sdk.is_order_scoring(order_id)
            return res.get("scoring", False)
        except Exception as e:
            logger.error(f"Error checking scoring for {order_id}: {e}")
            return False
