...
class DirectorAgent:
    ...
    async def evaluate_and_execute(self, cluster_alert: Dict) -> Dict:
        ...
        # Updated 2026-03-10: Added NBA and Tennis exclusions based on EV analysis
        # of 809 resolved trades. NBA: 33.3% accuracy, EV=-0.162 (n=234).
        # Tennis: 24.0% accuracy, EV=-0.269 (n=75). Both are proven capital destroyers.
        _esports_kw = [
        ...
        # A. Static Keywords (Legacy/Safety)
        ...
        # B. Dynamic Exclusions from .env (NBA, Tennis, etc.)
        if not _excluded_reason and settings.EXCLUDED_MARKET_CATEGORIES:
            excluded_cats = [c.strip().lower() for c in settings.EXCLUDED_MARKET_CATEGORIES.split(",") if c.strip()]
            for cat in excluded_cats:
                if cat in q_lower:
                    _excluded_reason = f"excluded_category_{cat}"
                    break
        
        # C. Specialized NBA/Tennis Keywords (Fallback guard)
        if not _excluded_reason:
            if any(k in q_lower for k in _nba_kw):
        ...
        try:
            ...
            # VPIN (Informed Flow Detection) kill switch
            # If one side of the book is >75% of total top-level liquidity, assume toxic flow/insider dumping.
            # VPIN = |bids - asks| / (bids + asks)
            bids_size = cluster_alert.get("clob_bids_size")
        ...
        # ── SCORE UPPER BOUND: >0.75 = Catastrophic Zone ─────────────────────
        # Empirical finding (2026-03-10): EV analysis of 809 resolved trades shows
        # council_score >0.75 has 0% accuracy in top bucket and EV=-0.584.
        # High scores indicate SentimentSwarm dominance or false consensus —
        # NOT genuine edge. Cap at 0.75 to avoid this failure mode.
        score_upper_bound = float(getattr(settings, 'AUTONOMOUS_CONFIDENCE_MAX', 0.75))
        if score > score_upper_bound:
            logger.warning(
                f"Director: Score {score:.3f} exceeds upper bound {score_upper_bound} "
                f"— catastrophic zone (EV=-0.584 in historical data). Skipping."
            )
            return {"status": "skipped", "reason": "score_above_upper_bound"}
        # ─────────────────────────────────────────────────────────────────────
        ...
        edge_brute = score - current_price
        edge_net = score - current_price - (spread * 0.5)
        ...
        if settings.PAPER_TRADING_MODE:
            ...
            if nofolio_triggered:
                agent_scores_dict["NoFolio"] = 1.0
